...
function generateHTML(habitsData, mode = 'smart') {
  ...
  const badgeText = mode === 'smart' ? 'SMART: Cache + Today' : 'FULL API FETCH';
  const badgeColor = mode === 'smart' ? '#48bb78' : '#f56565';
...
<head>
    ...
    <title>🚀 Smart Live Habit Streak</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .ring-bg {
            ...
            height: 100%;
            border-radius: 50%;
            background: conic-gradient(#48bb78 0% ${completionRate}%, #e2e8f0 ${completionRate}% 100%);
            display: flex;
            align-items: center;
            justify-content: center;
        ...
        .percentage { font-size: 32px; font-weight: bold; color: #2d3748; }
        .percentage-label { font-size: 11px; color: #718096; margin-top: -5px; }
        .habits-grid { display: flex; flex-direction: column; gap: 12px; margin-top: 20px; }
        .habit-row { background: #f7fafc; padding: 12px; border-radius: 12px; }
        .habit-header {
            display: flex;
        ...
        .habit-count { font-size: 11px; color: #718096; margin-left: 10px; }
        .days-strip {
            display: flex;
            gap: 3px;
            overflow-x: auto;
        ...
        .day-dot.completed { background: #48bb78; }
        .day-dot.in-progress { background: #ecc94b; }
        .day-dot.skipped { background: #cbd5e0; }
        .stats {
            display: flex;
        ...
        .timestamp { text-align: center; font-size: 10px; color: #a0aec0; margin-top: 15px; }
    </style>
</head>
<body>
    <div class="container">
        <div class="header">
            <div class="live-badge"><div class="live-dot"></div>${badgeText}</div>
            <h1>🎯 Habit Streak</h1>
        ...
        <div class="timestamp">
            🚀 ${mode === 'smart' ? '29 cached + today live' : 'All from API'} • ${new Date().toLocaleString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
        </div>
    </div>
</body>
</html>`;
...
module.exports = async (req, res) => {
  try {
    ...
    if (cachedHabitsData && cachedHabits) {
      // SMART MODE: Use cached past days + fetch only TODAY
      mode = 'smart';
      console.log('✨ Smart mode: Using cache + fetching today only');
      ...
      // Fetch only TODAY's data for all habits (8 API calls instead of 240!)
      for (const habitId of HABIT_IDS) {
        // Get cached data for this habit
        const cachedHabit = cachedHabitsData.find(h => h.id === habitId);
        let statuses = cachedHabit ? [...cachedHabit.statuses] : [];
        
        // Remove today if it exists in cache (to replace with fresh)
        statuses = statuses.filter(s => s.date !== todayStr);
        
        // Fetch fresh data for TODAY only
        try {
          const todayStatus = await getHabitStatus(habitId, today);
          statuses.push(todayStatus);
          console.log(`✓ Fetched today for ${habitsMap[habitId]}`);
        } catch (error) {
          console.error(`✗ Failed to fetch today for ${habitId}: ${error.message}`);
        }
      ...
      // FALLBACK: No cache, fetch all data from API (slow)
      mode = 'full';
      console.log('⚠️ No cache found, fetching all from API (slow)');
    ...
    const html = generateHTML(allHabitsData, mode);
    ...
    res.status(500).send(`<html><body style="font-family:system-ui;display:flex;align-items:center;justify-content:center;min-height:100vh;background:linear-gradient(135deg,#667eea,#764ba2);color:white;text-align:center;"><div><h1>⚠️ Error</h1><p>Failed to fetch data</p><p style="font-size:14px;margin-top:20px;">${error.message}</p></div></body></html>`);
  }
};
