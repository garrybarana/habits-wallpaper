...
function generateSVG(habitsData, width, height) {
  ...
  habitsData.forEach((habit, habitIndex) => {
    ...
    svg += `\n  <text x="${padding + habitNameWidth + habitsData[0].statuses.length * (cellSize + cellGap) + 20}" y="${y + 24}" font-size="15" font-weight="500" fill="#666666" font-family="Arial, sans-serif">${completed}</text>`;
  });
